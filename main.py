...
async def redirect_user(request: Request, uid: str = None):
    ...
    # ✅ Տանում է Poputi կայք
    return RedirectResponse(url="https://poputi.am")


# === TELEGRAM ԲՈՏ ===
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Բոտի start հրաման — գրանցում է մուտքը և ուղարկում ծանուցում ադմինին"""
    ...
    # ✅ Ադմինին ուղարկում ենք ծանուցում
    admin_msg = (
        f"🟢 Նոր մուտք բոտում\n"
        f"👤 Օգտատեր՝ @{user.username or '—'}\n"
        f"ID: {user.id}\n"
        f"Անուն: {user.first_name or ''} {user.last_name or ''}\n"
        f"Ժամանակ (UTC): {datetime.utcnow().isoformat()}"
    )
    for admin in ADMINS:
        try:
    ...
    # ✅ Օգտատիրոջը ուղարկում ենք հղումը
    text = (
